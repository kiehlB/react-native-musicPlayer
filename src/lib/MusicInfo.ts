...
class MusicInfoLoader {
  ...
  async loadFileToBuffer() {
    ...
    this.buffer.setData(Uint8Array.from(decode(data), (c: any) => c.charCodeAt(0)));
    this.filePosition += BUFFER_SIZE;
  }

  async loadInfo() {
    let info = (await FileSystem.getInfoAsync(this.fileUri)) as any;
  ...
  async processHeader() {
    let chunk = (await this.read(3)) as any;
  ...
  async processFrame() {
    ...
    if (frameID === EMPTY) this.finished = true;
      ...
      switch (frameID) {
        case TITLE_TOKEN:
          if (this.options.title) await this.processTextFrame(frameID, frameSize);
          else await this.skip(frameSize);
          break;
        case ARTIST_TOKEN:
          if (this.options.artist) await this.processTextFrame(frameID, frameSize);
          else await this.skip(frameSize);
          break;
        case ALBUM_TOKEN:
          if (this.options.album) await this.processTextFrame(frameID, frameSize);
          else await this.skip(frameSize);
          break;
        case GENRE_TOKEN:
          if (this.options.genre) await this.processTextFrame(frameID, frameSize);
          else await this.skip(frameSize);
          break;
        case PICTURE_TOKEN:
